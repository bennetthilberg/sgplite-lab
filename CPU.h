...
class CPU {
    ...
    void InitializeState() {
        ...
        // Fill the input buffer with random values so they can't cheat and
        // exploit the zeroes that would otherwise be here
        for (int i = 0; i < 4; i++) {
  ...
  public:
     ...
     */
    CPU(OrgWorld* world) : program(100), state{world} {
        InitializeState();
    }
     ...
     */
    const sgpl::Program<Spec>& GetProgram() const {
        return program;
    }
  ...
  private:
    ...
    void PrintOp(const sgpl::Instruction<Spec>& ins,
        ...
        if (arities.count(name)) {
            ...
            // Match the tag to the correct global anchor, then print it out as
            // a 2-letter code AA, AB, etc.
  ...
  public:
     ...
     * Output: None
     *
     * Purpose: Prints out a human-readable representation of the program code
     * of the organism's genome to standard output.
     */
    void PrintGenome(std::ostream& out = std::cout) {
        std::map<std::string, size_t> arities{
            {"Multiply", 3}, {"Add", 3}, {"Subtract", 3},
            {"Divide", 3},   {"IO", 1},  {"Reproduce", 0}};
